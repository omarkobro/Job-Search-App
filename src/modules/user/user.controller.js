...
export let signUp = async (req,res,next) =>{
    let {firstName, lastName ,email, recoveryEmail, password, role, mobileNumber , dateOfBirth}  = req.body
    ...
    let newUser = await User.create({firstName, lastName, userName:firstName+" "+lastName ,email, recoveryEmail, password: hashPassword, role, mobileNumber, dateOfBirth: new Date(dateOfBirth), })
    if(!newUser){
...
export let logIn = async (req,res,next) =>{
    ...
    let isEmailExists = await User.findOne({$or:[{email}, {mobileNumber}] });
    if (!isEmailExists) {
...
export let updateAccount = async (req,res,next) =>{
    ...
    let {email, mobileNumber, RecoveryEmail, firstName, lastName, dateOfBirth,role} = req.body
    if(email || mobileNumber){
        let isEmailExists = await User.findOne({$or:[{email}, {mobileNumber}] });
        if (isEmailExists) {
            return  next(new Error("Email or Phone Number is already in use try something else" , {casue:409}));
        }
    }
    let updatedUser = await User.findByIdAndUpdate(_id, {email, mobileNumber, RecoveryEmail, firstName, lastName, dateOfBirth,role},{new:true}
    )
...
export let resetPassword = async (req,res,next) =>{
    ...
    if(OTP && expiresIn >= Date.now()){
        ...
        tokenCheck.password = bcryptjs.hashSync(newPassword, +process.env.SALT_ROUNDS)
        tokenCheck.OTP = ""
...
export let getAccountsByRecoveryAccount = async(req,res,next) =>{
    ...
    if (!findUser.length) {
        return res.status(404).json({ message: 'No Accounts Associated With The Provided Recovery Email'});
    }
    res.status(200).json({message:"Accounts Associated With The Provided Recovery Email", findUser})
}